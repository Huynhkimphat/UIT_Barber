...
AS
BEGIN 
    INSERT INTO DICHVU(MADV,TENDICHVU, GIA, MOTADICHVU, HINHANH , MALDV) 
...
CREATE OR REPLACE PROCEDURE ADD_DATLICH(
    ...
    e_SERVICE       lstService,
    e_NGAY          DATLICH.NGAY%TYPE,
    e_MAGIO         DATLICH.MAGIO%TYPE,
    e_MANV          DATLICH.MANV%TYPE,
    e_TONGTIEN      HOADON.TONGTIEN%TYPE
)
AS
BEGIN
    --insert into HOADON
    INSERT INTO HOADON(MAHD, MAKH, NGAY, TONGTIEN, THANHTOAN) 
        VALUES(MAHD_SEQ11.NEXTVAL,e_MAKH,e_NGAY,e_TONGTIEN,0);
    --insert into DATLICH & CTHDDV
    FOR i IN 1..e_SERVICE.count LOOP
        --insert into DATLICH
        INSERT INTO DATLICH(MADL,NGAY,MAGIO,MAKH,MANV,MADV) 
            VALUES (MADL_SEQ10.nextval , e_NGAY , e_MAGIO , e_MAKH, e_MANV, e_SERVICE(i));
        --insert into CTHDDV
        INSERT INTO CTHDDV(MAHD,MADV,MADL) 
            VALUES(MAHD_SEQ11.CURRVAL,e_SERVICE(i),MADL_SEQ10.CURRVAL);
    END LOOP;
END;
                    ...
                    WHERE MAHD in ( 
                        ...
                        WHERE MADL in ( 
                            SELECT MADL FROM DATLICH    
                                WHERE       MAKH    = (SELECT MAKH FROM DATLICH WHERE MADL = e_MADL) 
                                    AND     NGAY    = (SELECT NGAY FROM DATLICH WHERE MADL = e_MADL)
                                    AND     MANV    = (SELECT MANV FROM DATLICH WHERE MADL = e_MADL) 
                                    AND     MAGIO   = (SELECT MAGIO FROM DATLICH WHERE MADL = e_MADL)
                        )
                    );
    UPDATE DATLICH SET TINHTRANG = 0 
                    WHERE MADL in (  
                        SELECT MADL FROM DATLICH  
                        WHERE   MAKH    = (SELECT MAKH FROM DATLICH WHERE MADL = e_MADL)  
                        AND     NGAY    = (SELECT NGAY FROM DATLICH WHERE MADL = e_MADL)  
...
AS
BEGIN
    ---soft delete to update----
    S_DELETE_DATLICH(e_MADL);
    ---add new DATLICH
...
IS
    ...
    e_TINHTRANG     LOAIDICHVU.TINHTRANG%TYPE;
    CUR_MADV        DICHVU.MADV%TYPE;
    CURSOR CUR IS SELECT DICHVU.MADV
                    FROM DICHVU
                    WHERE DICHVU.MALDV = e_MALDV AND DICHVU.TINHTRANG = 1;
BEGIN
